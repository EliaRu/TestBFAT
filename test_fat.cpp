...
int main( int argc, char *argv[] )
{
    ...
    auto C = [extra, &functions] 
    ( size_t key, uint64_t id, const tuple_t & a, const tuple_t& b, tuple_t& res )
    {
        functions.combine( key, id, a, b, res );
        if( extra > 0 ) {
    ...
    if( useFF ) {
        Win_Seq<tuple_t, tuple_t, tuple_t>* seq;
        if( fakeTB ) {
            seq = new Win_Seq<tuple_t, tuple_t, tuple_t>(
                functions.lift,
                C,
                functions.isCommutative,
                win_len,
                slide,
                quantum,
                "faketswinseq"
        ...
        Key_Farm<tuple_t, tuple_t>* kf;
        if( fakeTB ) {
            kf = new Key_Farm<tuple_t, tuple_t>( 
                functions.lift, 
                C, 
                functions.isCommutative, 
                win_len, 
