...
struct tuple_t
{
    ...
    // default constructor
    tuple_t(): key(0), id(0), ts(0), n( 0 ), value(0), secValue( 0 ), isZero( true ) {}
...
atomic_uint64_t num_tuples_generated = 0;
...
public:
    ...
    bool operator( )( tuple_t &t ) {
        if( !bandwith_test ) {
            ...
            if( first_tuple ) {
                base_time = t0 = t1 = current_time_nsecs( );
                first_tuple = false;
            } else {
                while( true ) {
...
public:
    // constructor
    Consumer(size_t _keys, operator_t<tuple_t>& _functions) : 
        received(0),
        noInBatch( 0 ),
        latency( 0 ),
        totalsum(0), 
    ...
    void operator( )( optional<tuple_t> &out ) {
        if( out ) {
            ...
            received++;
            functions.combine( 0, 0, acc, *out, acc );
            latency += ( t1 - ( out->ts + base_time ) );
